...
在非 go mod 项目下，使用相当于执行 git clone + go install

go mod 项目下，，依赖会被下载的到`GOPATH/pkg/mod`目录下的同时，也会在项目的 go.mod，go.sun 这个两个依赖描述文件中添加依赖的版本信息

## 版本控制

依赖的版本是使用 git 的 tag 来控制的，当`go get`的时候会优先使用最新 tag 对应的版本，即最新稳定版，你也可以指定版本号`go get -u xxx/xxx@1.0.1`
